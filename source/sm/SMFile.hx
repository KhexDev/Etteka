...
import haxe.Json;
import json.Section.SwagSection;
	...
	public function convertToFNF(saveTo:String):String
	{
		...
		for (measure in measures)
		{
			...
			for (i in 0...measure._measure.length - 1)
			{
				...
				/*if (!TimingStruct.getTimeFromBeat(currentBeat) is Float)
					{
						TimingStruct.addTiming(currentBeat, Conductor.bpm, currentBeat * 2, Conductor.offsets);
				}*/

				// seg = TimingStruct.getTimeFromBeat(currentBeat);
